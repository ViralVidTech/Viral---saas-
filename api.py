...
# ── FONCTION WAN 2.2 (legacy via WAN_API_URL) ────────────────────────────────
async def generate_wan_video(prompt: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=900) as client:
            response = await client.get(f"{base}/generate", params={"prompt": prompt})
...
def build_srt_from_timepoints(words: list, timepoints: list, out_path: str, words_per_block: int = 5):
    ...
    for tp in timepoints:
        ...
        if mark_name.startswith("w"):
            try:
                mark_times[int(mark_name[1:])] = float(tp.get("timeSeconds", 0.0))
            except ValueError:
                pass
    ...
    for j in range(0, len(words), words_per_block):
        ...
        end = (mark_times[next_j] - 0.05) if next_j in mark_times else (estimated_end - 0.05)
...
async def generate_audio_fish(req: FishTTSRequest):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return {"error": f"Fish Audio API erreur {response.status_code}", "details": details}
...
async def list_fish_voices():
    ...
    try:
        ...
        data = response.json()
        voices = [
            {"id": item.get("_id", ""), "name": item.get("title", ""),
             "language": item.get("languages", []), "description": item.get("description", "")}
...
# ── GENERATE : Claude + Pexels/Pixabay + Wan ─────────────────────────────────
@app.post("/generate")
...
""".strip()
    ...
    try:
        ...
        for line in lines:
            clean = line.strip()
            if clean.startswith("1."):   titles.append(clean[2:].strip())
            elif clean.startswith("2."): titles.append(clean[2:].strip())
            elif clean.startswith("3."): titles.append(clean[2:].strip())
            else:
        ...
        def extract_keywords(scene_text, fallback, scene_key=""):
            if not scene_text:
                return fallback
            stop_words = {
            ...
            keywords = [w.strip(".,!?;:") for w in scene_text.split()
                        if len(w.strip(".,!?;:")) > 4
                        and w.lower().strip(".,!?;:") not in stop_words]
            return (" ".join(keywords[:2]) + " " + fallback).strip() if keywords else fallback

        scene_queries = [extract_keywords(scenes[k], niche, k) for k in scene_keys if scenes[k]]
        while len(scene_queries) < 8:
            scene_queries.append(niche)

        async def fetch_pexels_videos(client, query, count=2):
            results = []
            try:
                ...
                for video in resp.json().get("videos", []):
                    ...
                    results.append((hd or files)[0].get("link", "") if (hd or files) else "")
            except Exception:
                pass
            return [u for u in results if u]

        async def fetch_pixabay_videos(client, query, count=2):
            results = []
            try:
                resp = await client.get(
        ...
        if WAN_API_URL:
            try:
                wan_video_url = await generate_wan_video(niche if niche else "funny talking character")
            except Exception:
                wan_video_url = ""
        ...
        return {
            ...
            "duration": duration,
            "video_url":   video_urls[0],  "video_url2":  video_urls[1],
            "video_url3":  video_urls[2],  "video_url4":  video_urls[3],
            "video_url5":  video_urls[4],  "video_url6":  video_urls[5],
            "video_url7":  video_urls[6],  "video_url8":  video_urls[7],
            "video_url9":  video_urls[8],  "video_url10": video_urls[9],
            "video_url11": video_urls[10], "video_url12": video_urls[11],
            "video_url13": video_urls[12], "video_url14": video_urls[13],
            "video_url15": video_urls[14], "video_url16": video_urls[15],
            "video_url17": video_urls[16], "video_url18": video_urls[17],
            "video_url19": video_urls[18], "video_url20": video_urls[19],
            "video_url21": video_urls[20], "video_url22": video_urls[21],
            "video_url23": video_urls[22], "video_url24": video_urls[23],
            "video_url25": video_urls[24], "video_url26": video_urls[25],
            "video_url27": video_urls[26], "video_url28": video_urls[27],
            "video_url29": video_urls[28], "video_url30": video_urls[29],
            "video_url31": video_urls[30], "video_url32": video_urls[31],
...
async def _process_video(job_id: str, req: VideoRequest):
    ...
    try:
        ...
        nb_scenes = {60: 8, 45: 6}.get(chosen_duration, 4)

        all_video_urls = [
            getattr(req, f"video_url{'' if i == 1 else i}", "").strip()
            for i in range(1, 41)
        ]
        all_subtitle_texts = [
            (getattr(req, f"text{i}", "") or "").strip()[:200]
            for i in range(1, 9)
        ]
        ...
        if req.wan_video:
            ...
            for i in range(nb_scenes):
                ...
                while len(collected) < CLIPS_PER_SCENE and valid_video_urls_raw:
                    collected.append(valid_video_urls_raw[(i * CLIPS_PER_SCENE + len(collected)) % len(valid_video_urls_raw)])
        ...
        await asyncio.gather(*[
            ...
            for url, path in zip(clip_urls, raw_paths) if url
        ])
        ...
        await async_run_cmd([
            ...
            "-c:v", "libx264", "-preset", "ultrafast", "-crf", "28", "-r", "25", "-pix_fmt", "yuv420p", "-an",
            stitched_raw
        ])
        ...
        if voice_url and voice_path:
            if music_url:
                ...
                await async_run_cmd([
                    ...
                    "-i", voice_path, "-stream_loop", "-1", "-i", music_path,
                    "-filter_complex",
                    f"[1:a]volume=0.12,atrim=0:{real_total_duration}[bg];[0:a][bg]amix=inputs=2:duration=first:dropout_transition=2[aout]",
        ...
        if sync_url:
            try:
                ...
                if words_list and timepoints_list:
                    srt_built = build_srt_from_timepoints(words_list, timepoints_list, srt_path)
            except Exception:
                srt_built = False

        if not srt_built:
            nb_subtitles = len([t for t in subtitle_texts if t.strip()])
            srt_segment_duration = (real_total_duration / nb_subtitles if nb_subtitles > 0 else clip_duration)
        ...
        if final_audio_path:
            await async_run_cmd([
                "ffmpeg", "-y", "-i", stitched_path, "-i", final_audio_path,
                "-vf", subtitle_filter, "-map", "0:v:0", "-map", "1:a:0",
                "-c:v", "libx264", "-preset", "ultrafast", "-crf", "28", "-r", "25",
                "-c:a", "aac", "-b:a", "128k", "-pix_fmt", "yuv420p", "-movflags", "+faststart",
                output_path
            ])
        else:
            await async_run_cmd([
                "ffmpeg", "-y", "-i", stitched_path, "-vf", subtitle_filter,
                "-c:v", "libx264", "-preset", "ultrafast", "-crf", "28", "-r", "25",
